...
{
    int retcode = 0;
    if (recv(sock_controll, &retcode, sizeof(retcode), 0) < 0){
...
int ftclient_get(int data_sock, int sock_control, char * arg)
{
    ...
    if (size < 0) {
        perror("error reading file is empty\n");
    }
...
{
    size_t num_recvd;                   //number of butes received with recv()
...
int ftclient_send_cmd(struct command * cmd)
{
    ...
    rc = send(sock_controll, buffer, (int)strlen(buffer), 0);
    if (rc < 0) {
...
void ftclient_login()
{
    ...
    int wait;
    recv(sock_controll, &wait, sizeof wait, 0);
...
int main(int argc, char * argv[])
{
    ...
    //Find an address to connect
    for (rp = res;  rp != NULL; rp = rp->ai_next) {
        sock_controll = socket(rp->ai_family, rp->ai_socktype, rp->ai_protocol);

        if(sock_controll < 0)
            continue;

        if (connect(sock_controll, res->ai_addr, res->ai_addrlen) == 0){
            break;
        } else {
    ...
    //Get connection, welcom messages
    ...
    while (1) {    // Loop until user types quit
        ...
        if (retcode == 502) {
            ...
            //opend data connection
            if ((data_sock = ftclient_open_conn(sock_controll)) < 0) {
            ...
            if (strcmp(cmd.code, "LIST") == 0) {
                ftclient_list(data_sock, sock_controll);
                ...
                ftclient_get(data_sock, sock_controll, cmd.arg);
                print_reply(read_reply());
            }
            close(data_sock);
